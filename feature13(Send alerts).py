...
def print_failed_services(failed_services):
    ...
    if failed_services:
        ...
        for service in failed_services:
            failed_urls.append(service["url"])

        print(f"Failed services: {', '.join(failed_urls)}")
    else:
        print("Failed services: None")


def build_alert_message(failed_services):
    lines = []
    lines.append("Server Health Checker Alert")
    lines.append("")
    lines.append("The following services failed health checks:")
    lines.append("")
    ...
    for service in failed_services:
        ...
        attempts = service.get("attempts", 1)
        error = service.get("error")

        line = f"- {url} | Status: {health_status} | HTTP: {status_code} | Attempts: {attempts}"

        if error:
            line = line + f" | Error: {error}"
...
def send_email_alert(failed_services):
    ...
    if not smtp_server or not smtp_port or not email_from or not email_to or not email_password:
    ...
    message["To"] = email_to
    message.set_content(build_alert_message(failed_services))
